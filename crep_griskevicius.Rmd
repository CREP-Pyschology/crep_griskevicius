...
> Note from Jordan: We need to update the deviations
...
>
> Question from Jordan: Should we also exclude age \< 18 yo?
...
> Jordan -\> write tetrachoric correlation for internal consistency of composite scale-\> or some way of addressing internal consistency. Note from Michał on collaborator draft: "can we report internal consistency reliability here for the composite score?" "I would like somebody more knowledgable with the matter to comment here, but I think that Cronbach Alpha can be used in such a case - it is just critical to use standardized alpha (not the"raw" one, as it will be biased in a case of a binary variable)"
...
1.  Make a variable that combines lab, lab individual and lab groups and compare results across that with in-person
...
> Note from Michał in google doc: this tool isn't really described in such a way that a reader could use it in their own research. I suggest to either describe it here in more detail, including the response format, or do it in the Appendix (I think this option will be even better, as those are exploratory analysis, so these should not occupy too much of the main body of the article).
...
> Suggestion from Aleksandra in doc: describe how dem/rep collapsed
