...
packages = c("tidyverse", "viridis", "colorspace","gghalves","ggthemes","skimr","Superpower","tidyr", "psych", "dplyr", "ggplot2", "sjmisc", "janitor", "rcompanion", "lsr", "papaja", "afex", "ltm","forestplot")
...
test.groups.car
usa.car.table <- table(usa.data$Groups.New, usa.data$greencar)
...
test.groups.soap
usa.soap.table <- table(usa.data$Groups.New, usa.data$greensoap)
...
test.groups.dishwasher
usa.dishwasher.table <- table(usa.data$Groups.New, usa.data$greendishwasher)
...
effect.time <- 
  structure(list(
    estimate  = c(NA,NA,cohen.14.estimate,cohen.16.estimate,cohen.17.estimate,cohen.18.estimate, cohen.19.estimate,cohen.20.estimate), 
    lower = c(NA,NA,cohen.14.lower,cohen.16.lower,cohen.17.lower, cohen.18.lower, cohen.19.lower, cohen.20.lower),
    upper = c(NA,NA, cohen.14.upper, cohen.16.upper, cohen.17.upper, cohen.18.upper, cohen.19.upper, cohen.20.upper)),
    .Names = c("estimate", "lower", "upper"), 
    row.names = c(NA,NA,"2014", "2016", "2017", "2018", "2019", "2020"), 
    class = "data.frame")

tabletext<-cbind(
  c(NA,"Year", "2014", "2016", "2017", "2018", "2019", "2020"))
...
forestplot(tabletext, 
           ...
           is.summary=c(TRUE,TRUE,rep(FALSE,11),TRUE),
           clip=c(-3,3), 
           title="TITLE",
           xlog=FALSE, 
...
What else? - effect over time, US versus rest
